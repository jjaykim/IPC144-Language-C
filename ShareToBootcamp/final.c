...
//double getTotalPrice(struct StockRecord* storeStock, int lange, int inputID, int inputQun)
...
//int getPositiveInt(struct StockRecord* storeStock, int lange, int inputID, int inputQun)
int getPositiveInt(struct StockRecord* storeStock, int validID, int inputQun)
{
  ...
  //    printf("the vlaue: %d\n", inputQun);
  //    // Set up to make a zero value if quantity is less than zero
  //    if (storeStock[i].salesRecord.amout - inputQun < 0)
  //    {
  //      convertNum = (storeStock[i].salesRecord.amout - inputQun) * -1;
  //      convertNum += (storeStock[i].salesRecord.amout - inputQun);
  //    }
  //    else
  //    {
  //      convertNum = storeStock[i].salesRecord.amout - inputQun;
...
int readSale(struct StockRecord storeStock[], int lange, struct SalesRecord saleItems[])
{
  ...
  do
  {
    ...
    else
    {
      ...
      {
        printf("Invalid Product - Enter a number between 0 and 14: ");  // I don't know how to display maximum value using lange
        scanf("%d,%d", &inputID, &inputQun);
      }
